...
public partial class auth_Adminpanel_ProcessManegment_ReserveUser_ReplyFromReserveToDfo : CrsfBase
{
    ...
    public static int? ForwardToStateUserID;
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!Page.IsPostBack)
        {
            ...
            DDlStatus.Items.FindByValue("4").Enabled = false;
        }
    }
    void QueryStringGet()
    {
    ...
    void GetRecordDfoReserveProcess(int TblFromReserveToDfoAutoID)
    {
        ...
        P_var.dSet = _tigerReserverBl.GetDfoReserveProcessDalReplynew(_TigerReserveOB);//GetDfoReserveProcessDalReplynew//GetDfoReserveProcessDal
        if (P_var.dSet.Tables[0].Rows.Count > 0)
        {
            ForwardToStateUserID = Convert.ToInt32(P_var.dSet.Tables[0].Rows[0]["ForwardToStateUserID"]);
                ...
                if (CurrentSessionId == hdnblank)
                {
                    ...
                    if (Page.IsValid)
                    {
                        ...
                        {//FromPersonID,ToPersonID,ToPersonUserName,FromPersonUserName,Action
                            ...
                            if (DDlStatus.SelectedValue == "1")//pending
                            {
                                ReplyCode();
                            }
                            ...
                            {//Verified only then previous accepted amount current txtaccepted amount must be 
                                ...
                                {
                                    Lblmsg.Text = "Your previous accepted amount is not match with current accepted amount.So that reason you choose status [" + StatusIdi + "].After discussion with comment then you can changed status [" + DDlStatus.SelectedItem.Text + "] to next senior level request!.";
                                    Lblmsg.ForeColor = System.Drawing.Color.Red;
                                }
                                else
                                {
