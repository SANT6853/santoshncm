...
public partial class auth_Adminpanel_Post_Addworkperform : System.Web.UI.Page
{
    ...
    public void BindScheme()
    {
        if (Request.QueryString["id"] != null)
        {
            ...
            p_Val.dSet = ngBL.GetWorkName(ngObj);
            if (p_Val.dSet.Tables[0].Rows.Count > 0)
            {
                txtExcutive.Text = p_Val.dSet.Tables[0].Rows[0]["ExecutiveName"].ToString();
            }
        }
    }
    [WebMethod]
    public static string SaveRecord(string WorkPerform)
    {
        ...
        if (Convert.ToInt32(HttpContext.Current.Session["ID"]) != null)
        {
            ...
            int Result = ngBL.SaveworkPerformHistory(_TigerReserveOB);
