...
public partial class UserControl_TopUserControlTigerReserve : System.Web.UI.UserControl
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        ...
        if (!IsPostBack)
        {
            ...
            {
                //Session["sTreserveName"] = Request.QueryString["trname"].ToString();
    ...
    public void BindRootMenu_Category()
    {
        try
        {
            ...
            if (Convert.ToInt16(Resources.NTCAResources.Lang_Id) == (int)Module_ID_Enum.Language_ID.English)
            {
                ...
                p_var.sbuilder.Append("</li>");
                p_var.sbuilder.Append("<li>");
                p_var.sbuilder.Append("<a title='" + HttpUtility.HtmlDecode(p_var.menu_name) + "'     href='" + ResolveUrl("~/ReportGP/") + "ReportsGP.aspx" + url + "'> Reports for General Public </a>");
                p_var.sbuilder.Append("</li>");
            }
                ...
                else
                {
                    if (p_var.dSet.Tables[0].Rows[p_var.i]["counter_Child"] != DBNull.Value)
                    {
                        ...
                        if (Convert.ToInt16(Resources.NTCAResources.Lang_Id) == (int)Module_ID_Enum.Language_ID.English)
                        {
                            ...
                            p_var.sbuilder.Append("<a title='" + HttpUtility.HtmlDecode(p_var.menu_name) + "'   href='" + ResolveUrl("~/") + "content/" + p_var.menuid + "_" + "1" + "_" + p_var.dSet.Tables[0].Rows[p_var.i]["UrlName"].ToString() + ".aspx" + "'>" + HttpUtility.HtmlDecode(p_var.menu_name) + "</a>");
                    ...
                    else
                    {
                            ...
                            else
                            {
                                ...
                                {//why relo
                                   // p_var.sbuilder.Append("<a title='" + HttpUtility.HtmlDecode(p_var.menu_name) + "' ='Homemiddlecontent.aspx?ModuleID=6&Websiteid=1&Link_Id=1021&bts=0'>"='Homemiddlecontent.aspx?ModuleID=6&Websiteid=1&Link_Id=1021&bts=0'>"  href='" + ResolveUrl("~/") + p_var.url + p_var.menuid + "_" + Convert.ToInt16(p_var.position) + "_" + p_var.urlname.Replace(" ", "").Replace("&", "and").Replace("'", "").Replace("_", "") + ".aspx'>" + Server.HtmlDecode(p_var.menu_name) + "</a>");
                                    ...
                                    {
                                        p_var.sbuilder.Append("<a title='" + HttpUtility.HtmlDecode(p_var.menu_name) + "'   href='" + ResolveUrl("~/") + p_var.url + p_var.menuid + "_" + Convert.ToInt16(p_var.position) + "_" + p_var.urlname.Replace(" ", "").Replace("&", "and").Replace("'", "").Replace("_", "") + ".aspx'" + url + "'>" + Server.HtmlDecode(p_var.menu_name) + "</a>");
                                    }
                                }
                            }
                        ...
                        else
                        {
                            ...
                            {
                                p_var.sbuilder.Append("<a title='" + HttpUtility.HtmlDecode(p_var.menu_name) + "'   href='" + ResolveUrl("~/content/") + p_var.url + p_var.menuid + "_" + Convert.ToInt16(p_var.position) + "_" + p_var.urlname.Replace(" ", "").Replace("&", "and").Replace("'", "").Replace("_", "") + ".aspx'"+url+">" + Server.HtmlDecode(p_var.menu_name) + "</a>");
                            }
